...
class SelectionService {
	async #getTwoRandomNumbers(count) {
		const rand1 = await Math.floor(
			mt.random(972348729347982347234927394) * count
		);
		const rand2 = await Math.floor(
			mt.random(227342349823412312371231232) * count
		);

		if (rand1 === rand2) {
			return this.#getTwoRandomNumbers();
		}
		return [rand1, rand2];
	}
	...
	async getRandomCharacters() {
		...
		const result1 = Character.findOne()
			.select(["-_id", "-__v", "-rating"])
			.skip(rand1);
		const result2 = Character.findOne()
			.select(["-_id", "-__v", "-rating"])
			.skip(rand2);
	...
	async updateCharacterRatings(winnerUrl, loserUrl) {
		...
		const [updatedWinnerRating, updatedLoserRating] =
			RatingService.getUpdatedRatings(winner.rating, loser.rating);
	...
	async getRandomAnimes() {
		...
		const result1 = Anime.findOne()
			.select(["-_id", "-__v", "-rating"])
			.skip(rand1);
		const result2 = Anime.findOne()
			.select(["-_id", "-__v", "-rating"])
			.skip(rand2);
